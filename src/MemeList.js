...
export default class MemeList extends Component<void, Props, State> {
  state: State = {
    data: [],
    page: {
      index: 0,
  ...
  _fetchAssets = async () => {
    await Expo.Font.loadAsync('impact', require('../assets/fonts/impact.ttf'));
  ...
  _fetchPopular = async () => {
    ...
    try {
      const res = await fetch(
        `http://version1.api.memegenerator.net/Generators_Select_ByPopular?pageIndex=${page.index}&pageSize=${page.size}&days=&apiKey=${config
          .memegenerator.key}`,
      );
      ...
      if (data.success) {
        this.setState(state => ({
          data: state.data.concat(data.result),
          status: 'loaded',
        }));
      } else {
        throw new Error();
  ...
  _renderRow = (rowData: any, { width }) => {
    return (
      <TouchableWithoutFeedback
        onPress={() => this.setState({ source: { uri: rowData.imageUrl } })}
      >
        <View>
          <Image
